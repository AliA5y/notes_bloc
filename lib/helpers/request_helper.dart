...
class RequestHelper {
  ...
  static Future<RequestState> handelDeviceId({bool updateUser = false}) async {
      ...
      if (deviceId == null) {
        ...
        try {
          await colRef.doc(_id).set({
            'userName': prefs.getString(PrefsKeys.nameKey),
            'userAvatar': prefs.getInt(PrefsKeys.avatarKey),
            'id': _id,
            'androidId': androidId,
        ...
        _id = deviceId;
        try {
          final doc = await colRef.doc(_id).get();
          doc['id'] == null;
          if (updateUser) {
            await colRef.doc(_id).set({
              'userName': prefs.getString(PrefsKeys.nameKey),
